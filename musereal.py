...
def warm_up(unet, vae, pe, device):
    # 预热函数
    batch_size = 16
    timesteps = torch.tensor([0], device=device)
    whisper_batch = np.ones((32, 50, 384), dtype=np.uint8)
    latent_batch = torch.ones(32, 8, 32, 32).to(device)
...
def inference(render_event, batch_size, input_latent_list_cycle, audio_feat_queue, audio_out_queue, res_frame_queue,
    ...
    warm_up(unet, vae, pe, unet.device)
